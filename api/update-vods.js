...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!twitchData.data) {
      return res.status(500).json({ error: 'No VOD data from Twitch' });
    }
    ...
    // Получаем существующие VOD из таблицы
    ...
    res.status(200).json({ success: true, added: twitchData.data.length });
  } catch (error) {
